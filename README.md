...
Observer Pattern: We used the Observer pattern in our project through the GameEvents script. In this script, several events are defined, such as OnHealthChanged, OnPlayerDied, and OnGameStarted. These events allow different scripts to communicate with each other without directly referencing one another. For example, in the Player script, the method TakeDamage() reduces the player’s health and then invokes the event GameEvents.OnHealthChanged. The UIManager script subscribes to this event using GameEvents.OnHealthChanged += SetHealth, which updates the health text on the UI whenever the player’s health changes. Similarly, when the player dies, the GameEvents.OnPlayerDied event is invoked, and the UIManager listens to this event to display the game over panel. This pattern was useful because it helped keep the scripts more organized and less dependent on each other. Instead of directly calling methods from other scripts, the game uses events to notify other systems when something happens. This makes the code easier to maintain and expand later. For example, if we want to add more reactions when the player dies, such as playing a sound or triggering an animation, we can simply subscribe another script to the same event without modifying the existing code.
